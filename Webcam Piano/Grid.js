    ...
    for (var i=0;i<this.notePos.length;i++){
      for (var j=0;j<this.notePos[i].length;j++){
        ...
        if (this.noteState[i][j]>0) {
          ...
          //Step 6: Idea 1 change the color effect
          var alpha = this.noteState[i][j] * 400; //make the note more visable
          var c1 = color(255,0,0,alpha); // more red tone
          var c2 = color(128,0,128,alpha); // more purple tone
